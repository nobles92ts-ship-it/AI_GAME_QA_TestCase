...
> **예외 (허용)**: 토스트·UI 텍스트 키 + 큰따옴표 문구 인용(`Feature_TOAST_X "보유한 아이템이 없습니다."`, `Tooltip_BTN_RateInfo "확률 정보"`)은 문구로 QA가 이해 가능하고 자동화 메시지 추적용이라 **허용**(기본기능 문구 인용 규칙과 동일). 금지 대상은 데이터 테이블·Enum·확률/수치 로직 변수(`RewardGroup`·`GroupProb`·`Feature_HighGradeCondition` 등)가 큰따옴표 문구 없이 주어/술어가 되는 경우.
