...
원천 섹션(트리 앞에 배치된 8종)의 각 항목이 **분류 그룹핑 트리의 `→` 케이스에 1회 이상 등장**하는지 기계적으로 대조. 누락 1건당 HIGH 1건.
...
추가 역방향 체크(선택):
- B-8 오류 패턴(분석.md): 각 패턴이 해당 소분류의 `→ 부정/예외`에 반영되었는가 → 미반영 시 MEDIUM
- C-1 미지정값(분석.md): 각 항목이 "기획 확인 필요 항목" + GlobalDefine `미지정` 컬럼에 모두 승격되었는가 → 미승격 시 MEDIUM

> **근거**: tc-설계.md 섹션 순서가 "원천 → 트리 → 배분·검증"으로 재정렬된 후, 검수는 단방향(원천 → 트리)으로 기계적 대조가 가능해짐. 학습 근거 P-05(교차 중복), P-08(상태 전이 엣지 누락) 재발 방지.
