...
| 역할 | 필수 로드 | 선택 |
|---|---|---|
| **writer (TC 신규 작성)** | 핵심 체크리스트 / 컬럼 구조 / TC 작성 규칙 / 기본기능 섹션 / 분류 그룹핑 / C~F열 검증 규칙 / 플랫폼 적용 / J열 화이트리스트 / F열 설계 태그 인용 금지 / F열 "또는" 표현 규칙 / 서식 규칙 / 작업 흐름 / 완료 전 자가 검증 / 완료 보고 형식 | 추후 구현 항목 |
...
- **데칼 없는 평타·논타겟(무조건 명중)**: 회피가 성립하지 않으므로 **부정(회피) 케이스를 만들지 않는다** — "소분류별 정상·부정·예외 최소 1개"의 예외(전투 프로필 한정).
...
5. **재현스탭**: 핵심 동작 확인 집중. **GlobalDefine 키 필수 인용** (예: `(Avatar_UI_Compound_Register_Info)`) + **기획서 안내 문구는 큰따옴표 그대로 인용**
...
| `버그ID-XXXX <설명>` | PASS/FAIL | N/A 등 | 확인된 기획서 버그 노트 (J열 한정) |
