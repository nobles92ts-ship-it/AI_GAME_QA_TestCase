...
실제 TC를 시트에 입력(수정, 재작성)할 때는 `{CLAUDE_SKILLS_DIR}\tc-생성\tc-생성.md`의 서식 규칙(정렬, 조건부서식, 드롭다운, 필터, 분류 그룹핑, 중복표기, 재현 스텝 한 문장 형식 등)을 전체 적용할 것.

**⚠️ 옵션 1-A 갱신 정책 [CRITICAL]**:
- **신규 추가 TC**: 옵션 1-A 적용 (정상=사전 상태 분류 자명 시 생략, 부정·예외=명시 필수, 미적용 예외 6종 준수)
- **기존 TC 변환 금지**: 갱신 대상이 아닌 기존 TC의 재현스탭은 **옵션 1-A 일괄 변환 금지** (회귀 차단). 사용자가 명시적으로 "기존 TC 옵션 1-A 일괄 변환" 요청하지 않는 한 보존.
- **기획 변경으로 수정되는 기존 TC**: 변경된 부분(수치/조건/기대결과)만 수정. 사전 상태 부분은 기존 형식 유지.
...
- **기존 탭 병합 원칙**: "기존 OO 탭에 추가/갱신해" 요청은 **신규 탭 생성 금지** — 해당 탭에 병합한다. 충돌 TC(동일 분류+검증단계)는 갱신하고, 신규 추가 TC의 ID는 텍스트로 입력(예: `'093`)해 기존 채번과 충돌을 막는다.
- **라이브 시트 기준 검증**: 갱신 영향범위·완전성 검증은 stale 가능성이 있는 로컬 스냅샷이 아니라 **갱신 직후 라이브 시트 재덤프** 기준으로 수행한다. 용어 개명/치환 작업은 "옛 용어 잔존 행 0건"을 라이브 덤프 grep으로 확인해야 완전 처리로 간주.
...
## 비고 변경 규칙 (J열 — 최근 2건만 유지)

> ⚠ 비고는 **J열**(10컬럼 구조 — tc-생성.md SSoT). K열은 완료처리가 쓰는 프로젝트 정보 패널이므로 절대 기입 금지.
