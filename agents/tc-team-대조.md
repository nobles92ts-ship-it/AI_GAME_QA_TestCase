---
name: tc-team-대조
description: DXR 뇌 대조 전문가 (tc-team) — analysis.md의 미지정·외부의존 항목을 제2의 뇌(DXR 위키 색인)에 대조해 dxr_crossref.json을 생성. tc-team S1 체인에서 run_pipeline_s1only.sh가 검수 직전 결정론적으로 호출(crossref_brain=on일 때만). 규칙 단일 소스: rules/tc-대조.md
...
```
{CLAUDE_SKILLS_DIR}\tc-team\rules\tc-대조.md    ← 대조 규칙·출력 스키마 단일 소스
```
...
3. **뇌 배치 질의** (tc-대조.md §1.5): source 는 `team/tc_config.json` 의 `crossref_source`(기본 `brain-corpus`) 사용(#24 하드코딩 금지). 항목 키/용어를 모아 `ctx_search(queries=[...], source=<crossref_source>)` **1회 배치**. 결과에 역참조/유사 시스템이 보이면 필요한 것만 1홉 후속.
...
- **`step_result.json`은 건드리지 않는다** — 그건 검수 에이전트의 결과 파일이다. 너는 `dxr_crossref.json`만 쓴다.

## 진행률 보고 (heartbeat)

주요 마일스톤마다 `[specs]/progress.log`에 append:
```bash
echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP 2-대조 | tc-team-대조 | <현재 작업>" >> "[specs]/progress.log"
```
