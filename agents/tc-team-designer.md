---
name: tc-team-designer
description: TC 설계 전문가 (tc-team) — 기획서 분석 + MD 파일 2개 생성 + 드라이브 업로드. tc-team S1(run_pipeline_s1only.sh)에서 호출됨. 규칙 단일 소스: rules/tc-분석.md(분석 단계) + rules/tc-설계.md(설계 단계)
...
```
1. {CLAUDE_SKILLS_DIR}\tc-team\rules\tc-분석.md   ← 분석 단계 규칙 (Step 1~7)
2. {CLAUDE_SKILLS_DIR}\tc-team\rules\tc-설계.md   ← 설계 단계 규칙 (Step 8~13)
```
...
> 단일 소스: `{CLAUDE_SKILLS_DIR}\tc-team\rules\tc-분석.md`
...
> 단일 소스: `{CLAUDE_SKILLS_DIR}\tc-team\rules\tc-설계.md`
...
> 이 에이전트는 얇은 포인터다. 모든 규칙은 두 규칙 파일(rules 서랍)이 단일 소스(Single Source of Truth)다.
...
```bash
echo "[$(date '+%Y-%m-%d %H:%M:%S')] STEP 1 | tc-team-designer | <현재 작업>" >> "$SPECS/[기능명]/progress.log"
```
...
> ⚠️ **수정 모드(STEP 3) 진입 시**: heartbeat 라벨에 `[FIX]` 접두 추가 (`STEP 3 | tc-team-designer [FIX] | <작업>`). 신규 작성과 수정의 시간 분리를 위해 필수.
