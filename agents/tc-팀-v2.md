...
4. 스킬 정의 완료 보고 형식으로 결과 기록 + **FINAL-6 이미지 매칭 안내** 한 줄을 보고 말미에 출력 (요청 TC 모두 완료 시 1회 / 이미지 실행은 안 함 — 안내만)
...
> 완료처리 내부 단계는 `FINAL-1~5` + `FINAL-6`(이미지 매칭 안내 — 실행 아님, 안내만) (파이프라인 STEP 1과 구분). 탭 색상 정렬은 대시보드 M3 버튼 수동 실행.
...
> 위 🖼 안내는 **요청받은 TC가 모두 완료된 뒤(단일=완주 시 / 배치=마지막 기능까지 완료 시) 1회만** 보고 말미에 붙인다 (완료처리 FINAL-6). 이미지 매칭 자체는 실행하지 않고 안내만 한다.
