...
| tc-설계검수-v2 | 2 | 설계 결과물 검수 | Opus --effort low |
| tc-designer-v2 | 3 | 설계 이슈 수정 (조건부) | Sonnet |
| tc-writer-v2 | 4 | TC JSON 생성 + 업로드 | **Haiku** |
| qa-reviewer-v2 | 5 | TC 구조 리뷰 | Sonnet |
| tc-fixer-v2 | 6 | 이슈 수정 + TC 추가/삭제 (조건부) | **Haiku** |
...
| 2 | tc-설계검수-v2 | opus | low | 10분 | 항상 | analysis.md, tc_design.md | design_review.md |
| 3 | tc-designer-v2 | sonnet | - | 30분 | needs_fix=true | design_review.md | analysis.md(수정), tc_design.md(수정) |
| 4 | tc-writer-v2 | haiku | - | 60분 | 항상 | tc_design.md | tc_snapshot.json, sheet_info.txt |
| 5 | qa-reviewer-v2 | sonnet | - | 10분 | 항상 | tc_snapshot.json | review_[탭명].md |
| 6 | tc-fixer-v2 | haiku | - | 30분 | total_issues>0 | review_[탭명].md | tc_after_fix1.json |
...
const spec={feature,state:newState,review_round:[N],spreadsheet_id:'[SHEET_ID]',tab_name:tabName,updated_at:new Date().toISOString()};
...
> **스키마 락**: `state` 값은 ALLOWED enum(9종)으로만 허용. 잘못된 값 시 exit 1로 즉시 중단. 손상된 specs 항목은 초기화 시 자동 제거.
...
const s=d.specs.find(x=>x.feature===process.env.FEATURE_NAME);
if(s){s[process.env.STEP_KEY]=new Date().toISOString();fs.writeFileSync(f,JSON.stringify(d,null,2));}" 2>/dev/null
```

> `updated_at`(시작) + `step[N]_completed_at`(종료) 짝으로 소요시간 산출.
...
> tab_name은 STEP 4 완료 전까지 빈 문자열. STEP 4 완료 후 step_result.json에서 읽어 반영.
...
bash "$RETRY" "$SPECS/[기능명]/step2_stderr.log" -- \
  claude -p --agent tc-설계검수-v2 --model opus --effort low --permission-mode bypassPermissions "
## HANDOFF
- 기능명: [기능명]
- specs 경로: $SPECS/[기능명]
...
bash "$RETRY" "$SPECS/[기능명]/step4_stderr.log" -- \
  claude -p --agent tc-writer-v2 --model haiku --permission-mode bypassPermissions "
## HANDOFF
- 기능명: [기능명]
- 스프레드시트 ID: [SHEET_ID]
- 탭명: [탭명]
...
"
```
→ `cp step_result.json step5_result.json` → `total_issues>0` 시 STEP 6, 아니면 STEP 7

#### STEP 6 — 1차 수정 (조건부, Haiku)
...
if [ "$ATTEMPTS" -ge 1 ]; then
  ...
  bash "$RETRY" "$SPECS/[기능명]/step6_stderr.log" -- \
    claude -p --agent tc-fixer-v2 --model haiku --permission-mode bypassPermissions "
## HANDOFF
- 기능명: [기능명]
- 스프레드시트 ID: [SHEET_ID]
- 탭명: [탭명]
...
"
```
→ `cp step_result.json step7_result.json` → 완료 처리
...
| 배치 누적 $30 초과 | cost.log 합산 | 사용자 계속 여부 확인 | — |
| 단일 기능 $10 초과 | cost.log 단일 합산 | 루프 의심, 즉시 중단 | — |

### STEP별 적용 정책

| STEP | S1 쿼터 | S2 백업 | S3 컬럼검증 | S4 토큰 | S5 비용 | S6 재시도 | S7 heartbeat |
|---|---|---|---|---|---|---|---|
| 1 | - | - | - | ✓ | ✓ | 기타 | ✓ |
| 2 | - | - | - | ✓ | ✓ | 기타 | ✓ |
| 3 | - | - | - | ✓ | ✓ | 기타 | ✓ |
...
| 정책 | 상세 위치 |
|---|---|
| S1 쿼터 | `$STABILITY_DOC#s1` |
| S2 백업 탭 | `$STABILITY_DOC#s2` |
| S3 컬럼 검증 | `$STABILITY_DOC#s3` + `tc-fixer-v2.md` |
| S4 토큰 | `$STABILITY_DOC#s4` |
| S5 비용 | `$STABILITY_DOC#s5` |
| S6 에러 분류 | `$STABILITY_DOC#s6` |
| S7 heartbeat | `$STABILITY_DOC#s7` |
