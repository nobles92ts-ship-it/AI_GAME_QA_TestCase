...
const BLOCK_HEIGHT    = 7;    // 섹션헤더(1) + 플랫폼(1) + 데이터(5)
const BLOCK_GAP       = 2;    // 블록 사이 빈 행 수
const TITLE_ROWS      = 2;    // 타이틀(1) + 빈 행(1)

const PLAT_COLS = { PC: 'H', 모바일: 'I' };
const PLATFORMS  = ['PC', '모바일'];
const METRICS    = ['PASS', 'FAIL', 'BLOCK', '미진행', '합계'];
const ROW_LABELS = ['✅ PASS', '❌ FAIL', '🚫 BLOCK', '⏸ 미진행', '합계'];
...
const sumFormula = (sheet, col) =>
  `SUMPRODUCT(COUNTIF(${ref(sheet)}!${col}:${col},{"PASS","FAIL","BLOCK","미진행"}))`;
...
async function updateDashboard() {
  ...
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId: SPREADSHEET_ID,
    requestBody: { requests: [{ unmergeCells: { range: R(SID, 0, 0, 200, 60) } }] }
  });
  ...
  // ── 진행률 패널 (통합 블록 우측, 수식 기반 자동 갱신) ──────────────────
  {
    ...
    updates.push({ range:`${DASHBOARD_NAME}!${pCB}${pr0+1}`, values:[[
      `=SPARKLINE({C${pcR},C${pfR},C${pbR},C${pmR}},{"charttype","bar";"color1","#4285F4";"color2","#EA4335";"color3","#212121";"color4","#BDBDBD";"max",C${psR}})`
    ]] });
    updates.push({ range:`${DASHBOARD_NAME}!${pCB}${pr0+2}`, values:[[
      `=IFERROR(TEXT(C${pcR}/C${psR},"0.0%")&" PASS  /  "&TEXT(C${pfR}/C${psR},"0.0%")&" FAIL","미진행")`
    ]] });

    // 모바일
    updates.push({ range:`${DASHBOARD_NAME}!${pCL}${pr0+3}`, values:[['모바일']] });
    updates.push({ range:`${DASHBOARD_NAME}!${pCB}${pr0+3}`, values:[[
      `=SPARKLINE({D${pcR},D${pfR},D${pbR},D${pmR}},{"charttype","bar";"color1","#4285F4";"color2","#EA4335";"color3","#212121";"color4","#BDBDBD";"max",D${psR}})`
    ]] });
    updates.push({ range:`${DASHBOARD_NAME}!${pCB}${pr0+4}`, values:[[
      `=IFERROR(TEXT(D${pcR}/D${psR},"0.0%")&" PASS  /  "&TEXT(D${pfR}/D${psR},"0.0%")&" FAIL","미진행")`
    ]] });
  }
  ...
  for (let k = 0; k < chunks.length; k++) {
    ...
    ROW_STYLES.forEach(({ label, cell }, mi) => {
      ...
      if (hasTotal) {
        ...
        if (numSec > 1) {
          reqs.push(repeat(R(SID, ri, sCol(1), ri+1, blockEndCol), cellFmt(cell, C.BLACK, mi===4, 'CENTER', 'MIDDLE', 'CLIP')));
        }
      } else {
        reqs.push(repeat(R(SID, ri, sCol(0), ri+1, blockEndCol), cellFmt(cell, C.BLACK, mi===4, 'CENTER', 'MIDDLE', 'CLIP')));
      }
    });
  ...
  // ── 진행률 패널 서식 ────────────────────────────────────────────────────
  {
    ...
    // 나머지 행 (흰색)
    reqs.push(repeat(R(SID, pb0+5, pC, pb0+7, pC+2), cellFmt(C.WHITE)));

    // 전체 테두리
    reqs.push(addBorder(SID, pb0, pC, pb0+7, pC+2, true));
  ...
  const ROW_HEIGHTS = [50, 30, 28, 28, 28, 28, 30];
  for (let k = 0; k < chunks.length; k++) {
    const base = blockStartRow(k);
