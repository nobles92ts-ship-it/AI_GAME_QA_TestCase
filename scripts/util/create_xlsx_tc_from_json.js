 ...
 * expectedHI 로 PC/모바일 결과)을 복제하되, 시트 업로드 대신 로컬 엑셀 파일로 저장한다.
 * 이미 설치된 의존성(xlsx)만 사용하므로 OAuth/구글 설정이 전혀 필요 없다.
 *
 * 입력 tc_data.json: [["대분류","중분류","소분류","검증단계","재현스탭","플랫폼","비고(optional)"], ...]
 *   ※ 헤더 행 포함 금지 — 데이터 행만.
 *
 * 모듈 사용:  const { writeXlsx } = require('./create_xlsx_tc_from_json'); writeXlsx(기능명, 출력경로, tcData)
 * CLI 사용:   node create_xlsx_tc_from_json.js "기능명" "출력경로.xlsx" "tc_data.json경로"
 */
const fs = require('fs');
...
/** tcData(데이터 행 배열) 를 outPath 로 .xlsx 작성. 절대경로 반환. */
function writeXlsx(feature, outPath, tcData) {
  if (!Array.isArray(tcData) || tcData.length === 0) throw new Error('tc_data 가 비었거나 배열이 아닙니다.');
  const ws = XLSX.utils.aoa_to_sheet(buildRows(tcData));
  ws['!cols'] = [
    { wch: 7 }, { wch: 12 }, { wch: 14 }, { wch: 16 }, { wch: 9 },
    { wch: 64 }, { wch: 10 }, { wch: 9 }, { wch: 11 }, { wch: 16 }, { wch: 9 },
  ];
  const wb = XLSX.utils.book_new();
  const tab = String(feature).replace(/[\\/?*\[\]:]/g, '_').substring(0, 31) || 'TC';
...
module.exports = { writeXlsx, buildRows, HEADER };

if (require.main === module) {
  const FEATURE = process.argv[2], OUT_PATH = process.argv[3], JSON_FILE = process.argv[4];
  if (!FEATURE || !OUT_PATH || !JSON_FILE) {
    console.error('사용법: node create_xlsx_tc_from_json.js "기능명" "출력경로.xlsx" "tc_data.json경로"');
    process.exit(1);
  }
  let tcData;
  try { tcData = JSON.parse(fs.readFileSync(JSON_FILE, 'utf8')); }
  catch (e) { console.error(`[ERROR] tc_data.json 읽기 실패: ${e.message}`); process.exit(2); }
  try { const p = writeXlsx(FEATURE, OUT_PATH, tcData); console.log(`[OK] ${tcData.length}개 TC → ${p}`); }
