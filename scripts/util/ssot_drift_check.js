...
// 3) EVAL-16 / V-18 = 5조건
// 2026-09-03 기본기능 틀 개편: 5조건 → 6조건(검증단계=정상 고정 추가). 개수 변경에 안 깨지도록 N가지로 완화
need('리뷰', 'EVAL-16 N가지 충족 문구', has('리뷰', '가지 모두 충족'));
need('생성', 'V-18 N가지 충족 문구', has('생성', '가지 모두 충족'));
// 기본기능 정상 전용 — 4종 SSoT + 다이제스트가 같은 규칙을 말하는지 (이번 개편의 회귀 감시)
need('설계', '기본기능 정상 전용', has('설계', '`정상` 고정'));
need('생성', '기본기능 정상 전용', has('생성', '`정상` 고정'));
need('리뷰', '기본기능 정상 전용', has('리뷰', '`정상` 고정'));
