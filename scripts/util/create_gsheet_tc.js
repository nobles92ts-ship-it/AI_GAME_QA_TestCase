 ...
 *
 * 사용법:
 *   node create_gsheet_tc.js "탭명" "스프레드시트ID"
 *
 * 동작:
...
if (!TAB_NAME || !MASTER_SPREADSHEET_ID) {
    console.error('사용법: node create_gsheet_tc.js "탭명" "스프레드시트ID"');
    process.exit(1);
}
...
async function createTcTab() {
    ...
    function tc(major, minor, sub, verif, platform, step, assignee = '') {
        ...
        rowsData.push([id, dMajor, dMinor, dSub, verif, step, platform, pcResult, mobileResult, '', assignee]);
    }

    // ── TC 데이터 (tc-writer 에이전트가 여기에 tc() 호출을 삽입) ──
    // 예시:
    // tc('대분류', '중분류', '소분류', '정상', 'PC', '재현 스텝 내용', '클라');
