...
// J열 화이트리스트 — 리터럴 4종 + DXBUG 패턴 (플레이스홀더 'DXBUG-XXXX' 자체는 위반 — L4-09)
const J_WHITELIST = ['', '추후 구현', '구현 우선순위 낮음', '기획 확인 필요'];
const J_DXBUG_RE = /^DXBUG-\d+$/;
 ...
 *  - 그 외(빈값·구현 우선순위 낮음·기획 확인 필요·DXBUG-####):
...
function validateFull(filePath, opts = {}) {
  ...
  records.forEach((r, i) => {
    ...
    // V-19 J 화이트리스트
    if (!isJAllowed(r.J)) {
      violations.push({ sev: 'HIGH', rule: 'V-19', row: r.sheetRow, msg: `${loc} J열 "${r.J}" — 화이트리스트 외${normJ(r.J) === 'DXBUG-XXXX' ? ' (플레이스홀더 리터럴 금지)' : ''}` });
    }
  ...
  // 배분표 선파싱 + [근거부족—기획질의] 면제 소분류 수집 (2026-07-28 면제 단일화 — tc-설계 §면제 형식 3-1)
  //   토큰 행 = 기계가 미달 판정에서 제외 (질의 승격 확인은 EVAL-02 소관). 대시 표기 편차(—/–/-) 허용.
  const EXEMPT_RE = /\[근거부족\s*[—–-]\s*기획질의\]/;
  let design = null;
  const exemptKeys = new Set();
  if (opts.designPath) {
    design = parseDesignTables(opts.designPath);
    if (design.ok && design.allocation) {
      for (const e of design.allocation) {
        if (e.note && EXEMPT_RE.test(e.note)) exemptKeys.add(`${e.B}|${e.C}|${e.D}`);
      }
    }
  }
  if (exemptKeys.size) notes.push(`[근거부족—기획질의] 면제 소분류 ${exemptKeys.size}개 — V-10 하한·배분 대조·V-16r 미달 판정 제외 (질의 승격 확인=EVAL-02)`);

  // V-10 전체 하한: 부정+예외 ≥ 49% (기본기능 제외 분모, 면제 소분류 행 제외 — 2026-07-28 escape)
  const qaFloor = exemptKeys.size ? qa.filter(r => !exemptKeys.has(`${r.B}|${r.C}|${r.D}`)) : qa;
  const floorStage = { 정상: 0, 부정: 0, 예외: 0 };
  qaFloor.forEach(r => { if (STAGE_ENUM.includes(r.E)) floorStage[r.E]++; });
  const floorTotal = floorStage.정상 + floorStage.부정 + floorStage.예외;
  const floorRatio = floorTotal ? Math.round(((floorStage.부정 + floorStage.예외) / floorTotal) * 1000) / 10 : 0;
  if (floorTotal > 0 && floorRatio < 49) {
    violations.push({ sev: 'HIGH', rule: 'V-10', msg: `부정+예외 비율 ${floorRatio}% < 49% 하한 (QA ${floorTotal}행 기준${exemptKeys.size ? `, 면제 소분류 ${exemptKeys.size}개 제외` : ''})` });
  }

  // 배분표 대조 (V-10/V-16리스크 — design 제공 시, 위에서 선파싱)
  ...
  if (opts.designPath) {
    if (!design.ok || !design.allocation) {
      ...
      for (const [key, st] of subStats) {
        ...
        e._matched = true;
        // [근거부족—기획질의] 토큰 = 기계 면제 (미달을 이슈로 잡지 않음 — 질의 승격 확인은 EVAL-02).
        // 형식 밖 자유 서술 비고는 기존대로 llmFlag (2026-07-28 단일화 이후 무효 후보 — LLM이 판정)
        const exempted = exemptKeys.has(key);
        // 단계별 미달 (부등식: 실제 ≥ 배분표 — 초과는 합법 L3-1)
        [['정상', e.normal], ['부정', e.neg], ['예외', e.exc]].forEach(([stage, want]) => {
          if (want === null || want === undefined) return;
          if (st[stage] < want) {
            if (exempted) notes.push(`(${key}) ${stage} ${st[stage]} < 배분 ${want} — [근거부족—기획질의] 면제`);
            else if (e.note) llmFlags.push({ rule: 'V-10', reason: `(${key}) ${stage} ${st[stage]} < 배분 ${want}, 배분표 비고 "${e.note}" — 사유 적격성 LLM 판정 (L3-4, ⚠면제 단일화: [근거부족—기획질의] 형식 외 자유 서술은 무효 후보)` });
        ...
        if (tot > 0 && e.risk) {
          ...
          if (th !== null && ratio < th - 0.5) {
            ...
            if (exempted) notes.push(`(${key}) [${e.risk}] 부정+예외 ${Math.round(ratio)}% < ${th}% — [근거부족—기획질의] 면제`);
            else if (e.note) llmFlags.push({ rule: 'V-16r', reason: `(${key}) [${e.risk}] 부정+예외 ${Math.round(ratio)}% < ${th}%, 비고 "${e.note}" — LLM 판정 (⚠면제 단일화: [근거부족—기획질의] 형식 외 자유 서술은 무효 후보)` });
        ...
        if (st.정상 === 0 && (e.normal === null || e.normal > 0)) {
          if (exempted) notes.push(`(${key}) 정상 0건 — [근거부족—기획질의] 면제`);
          else if (e.note) llmFlags.push({ rule: 'V-10', reason: `(${key}) 정상 0건, 배분표 비고 "${e.note}" — LLM 판정` });
...
const TREE_EXTRA_TAGS = new Set(['데이터', '동시성', '세션', 'UI', '이미지 참조 필요', '연계', '유저상태', '권한', '시간조작', '대소문자']); // §3.1 암묵적 요구사항 태그 전종(9종) 등재 — 시간조작·대소문자 추가(2026-07-09)
