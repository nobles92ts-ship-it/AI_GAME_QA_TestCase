 ...
 * TC 팀 v2 파이프라인 착수 시 your-qa-channel 채널에 "TC 생성 요청" 공지를 전송한다.
 *
 * 사용법:
 *   # 단일 기능
...
const DEFAULT_CHANNEL = 'YOUR_SLACK_CHANNEL_ID'; // your-qa-channel
...
async function main() {
  ...
  if (cfg.dedupDir) {
    ...
    if (fs.existsSync(marker)) {
      console.log(`[your-qa-channel] 동일 요청 이미 전송됨 — 스킵 (${path.basename(marker)})`);
      return;
    }
  }
  ...
  console.log(`[your-qa-channel] TC 생성 요청 공지 전송 완료 (${items.length}건): ${names}`);
}
