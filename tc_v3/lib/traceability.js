...
const EXCLUSION_REASONS = ['추후구현', '타기획서', '비TC성서술', '중복규칙'];

/**
 * @param rules      [{rule_id, sec_id, text}]           slicer.rules
 * @param coverage   {rule_id: [tc_id,...]} | [{rule_id, tc_ids}]
 * @param exclusions [{rule_id, reason}]   reason ∈ EXCLUSION_REASONS
 */
function buildLedger(rules, coverage, exclusions = []) {
  const covMap = Array.isArray(coverage)
    ? Object.fromEntries(coverage.map(c => [c.rule_id, c.tc_ids || []]))
    : (coverage || {});
  const exMap = Object.fromEntries((exclusions || []).map(e => [e.rule_id, e.reason]));
