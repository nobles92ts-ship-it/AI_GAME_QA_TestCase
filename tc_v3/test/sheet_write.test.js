...
t('베이스 탭 비어있으면 생성', () => {
  const created = [];
  const res = resolveAndWrite(t => (created.push(t), { exit: 0 }), () => {}, { baseTab: 'sample_feature', runId: 'r1', marker: null });
  assert.ok(res.ok);
  assert.strictEqual(res.tab, 'sample_feature');
  assert.strictEqual(res.action, 'created');
});

t('베이스 탭 점유(남의 것) → _v2로 bump', () => {
  const res = resolveAndWrite(
    t => t === 'sample_feature' ? { exit: 3 } : { exit: 0 },
    () => {},
    { baseTab: 'sample_feature', runId: 'r1', marker: null });
  assert.ok(res.ok);
  assert.strictEqual(res.tab, 'sample_feature_v2');
  assert.strictEqual(res.action, 'created');
});

t('_v2도 점유 → _v3', () => {
  const taken = new Set(['sample_feature', 'sample_feature_v2']);
  const res = resolveAndWrite(t => taken.has(t) ? { exit: 3 } : { exit: 0 }, () => {}, { baseTab: 'sample_feature', runId: 'r1', marker: null });
  assert.strictEqual(res.tab, 'sample_feature_v3');
});
...
t('우리 소유 탭 존재(마커) → 삭제 후 재기록(clear_and_rewrite)', () => {
  ...
  const res = resolveAndWrite(
    t => (t === 'sample_feature_v2' && !deletedFlag) ? { exit: 3 } : { exit: 0 },
    t => { deleted.push(t); deletedFlag = true; },
    { baseTab: 'sample_feature', runId: 'r1', marker: { run_id: 'r1', tab: 'sample_feature_v2' } });
  assert.ok(res.ok);
  assert.strictEqual(res.tab, 'sample_feature_v2');
  assert.strictEqual(res.action, 'clear_and_rewrite');
  assert.deepStrictEqual(deleted, ['sample_feature_v2']);
});
...
t('다른 run의 마커는 재사용 안 함(내 것만 clear)', () => {
  ...
  const res = resolveAndWrite(
    ...
    { baseTab: 'sample_feature', runId: 'r2', marker: { run_id: 'r1', tab: 'sample_feature' } });
  assert.strictEqual(res.tab, 'sample_feature_v2');
});
