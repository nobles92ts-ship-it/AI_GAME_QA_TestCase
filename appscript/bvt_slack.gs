...
// 대시보드 M6 체크박스 → BVT(Trunk) 테스트 환경 정보를 your-qa-channel 채널로 전송
...
const SLACK_CHANNEL = 'YOUR_SLACK_CHANNEL_ID'; // your-qa-channel (qa-widget과 동일)
...
function setupM6Button() {
  ...
  btn.setValue(false);
  btn.setNote('체크 → BVT(Trunk) 결과를 your-qa-channel 채널로 전송');
