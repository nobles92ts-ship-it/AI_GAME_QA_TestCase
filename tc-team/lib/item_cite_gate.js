 ...
 *        `단단한 강철 주괴`(고유 실명)는 Index 가 필요 없다. 이 게이트를 실 시트에 처음 걸었을 때 낸 오탐이다.
 ...
 *      "모든 성장 재료가 …" 에 `(예: 세공석)` 을 붙이면 테스터가 1건만 보고 넘긴다 = 범위 축소 오독.
 ...
 *   `(예: 제작 재료 임시 명칭(Index 301000001))`. `[^)]*` 류는 안쪽 `)` 에서 잘려
 *   이름이 "…명칭(Index 301000001" 로 남고, 그 뒤 모든 판정이 오탐이 된다(실측 6건).
...
function checkItemCite(rows, dict) {
  ...
  for (const r of rows) {
    ...
    if (cites.length) {
      ...
      //   실측: `단단한 강철 주괴`(고유 실명)를 "제작 재료" 유형이 placeholder 라는 이유로 위반 처리했다.
      let bad = false;
