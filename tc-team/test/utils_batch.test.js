...
t('covered+excluded 조합 = 게이트 통과', () => {
  const r = buildLedger(rules, { 'R-1.1': ['001'], 'R-1.2': ['002', '003'] }, [{ rule_id: 'R-2.1', reason: '타기획서' }]);
...
t('추후구현 제외 = 차단 (M-021 — TC화 강제, v2 tc-생성 승계)', () => {
  const r = buildLedger(rules, { 'R-1.1': ['1'], 'R-1.2': ['2'] }, [{ rule_id: 'R-2.1', reason: '추후구현' }]);
  assert.strictEqual(r.gate.pass, false, '추후구현은 더 이상 합법 제외 사유가 아님');
  assert.strictEqual(r.badExclusion.length, 1);
});

t('dangling coverage(미존재 rule_id 참조) = 차단', () => {
  const r = buildLedger(rules, { 'R-1.1': ['1'], 'R-1.2': ['2'], 'R-9.9': ['x'] }, [{ rule_id: 'R-2.1', reason: '타기획서' }]);
  assert.strictEqual(r.gate.pass, false);
...
t('신규 feature upsert', () => {
  const s = project({}, '자동_사냥', 'S3', null, null);
