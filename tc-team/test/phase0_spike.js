...
const SPEC = '{PROJECT_ROOT}/team/specs/자동_사냥_기능';
let pass = 0, fail = 0;
...
t('② traceability — 실물 규칙에 커버리지 게이트 기동', () => {
  ...
  rules.forEach((r, i) => {
    ...
    if (i % 10 === 8) { ex.push({ rule_id: r.rule_id, reason: '타기획서' }); return; } // 제외 (M-021: 추후구현은 제외 불가로 전환)
