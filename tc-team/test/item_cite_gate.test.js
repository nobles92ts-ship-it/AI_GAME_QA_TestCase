...
const DICT = {
  systems: [
    { key: '세공', item_count: 3, examples: [{ index: 303000001, name: '세공석', name_placeholder: false, cite: '세공석' }] },
    { key: '제작', item_count: 60, examples: [{ index: 301000001, name: '제작 재료 임시 명칭', name_placeholder: true, placeholder_reason: 'shared', cite: '제작 재료 임시 명칭(Index 301000001)' }] },
  ],
  types: [
    { inventory_category: '재료', item_type: '성장 재료', examples: [{ index: 302010001, name: '일반 무기 강화 주문서', name_placeholder: false, cite: '일반 무기 강화 주문서' }] },
  ...
  name_index: { '세공석': 303000001, '일반 무기 강화 주문서': 302010001, '단단한 강철 주괴': 301000005, '순간 이동 주문서': 304000001 },
};
...
t('④ 유형만 지목 → missing_cite 후보 + 병기 후보를 제시한다', () => {
  ...
  assert.ok(/세공석/.test(m[0].detail), '사전 예시를 후보로 제시');
  assert.ok(/축1/.test(m[0].detail), '판정 축을 명시해 LLM 이 무엇을 결정해야 하는지 알려준다');
});

t('병기가 있으면 후보로 잡지 않는다 (already_cited 로 센다)', () => {
  const { findings, excluded } = checkItemCite([R('011', '세공 재료 아이템(예: 세공석)을 사용하면 세공 화면으로 전환되는지 확인')], DICT);
...
t('② 전수 검증 문장에 대표 1건 병기 = HIGH (범위 축소 오독)', () => {
  const { findings } = checkItemCite([R('200', '모든 성장 재료 아이템(예: 일반 무기 강화 주문서)의 툴팁에 사용 버튼이 출력되는지 확인')], DICT);
...
t('① Index 가 붙어 있으면 통과 (중첩 괄호 파싱 — 실측 오탐 6건 회귀 방지)', () => {
  const { findings, excluded } = checkItemCite([R('001', '제작 재료 아이템(예: 제작 재료 임시 명칭(Index 301000001)) 툴팁에 제작 아이콘이 출력되는지 확인')], DICT);
...
t('① 판정은 유형이 아니라 **인용된 개체** 기준 — 같은 유형의 고유 실명은 Index 불필요 (실측 오탐 회귀 방지)', () => {
  const { findings } = checkItemCite([R('042', '제작 재료에만 해당하는 아이템(예: 단단한 강철 주괴)의 툴팁을 열면 사용 버튼이 출력되지 않는지 확인')], DICT);
...
t('③ 한 문장에 같은 병기 2회 → MEDIUM', () => {
  const { findings } = checkItemCite([R('092', '세공 재료 아이템(예: 세공석)을 사용한 뒤 같은 세공 재료 아이템(예: 세공석)을 다시 사용하면 재판정되는지 확인')], DICT);
...
t('`, 예: X)` 형태(부정 TC 괄호 안 병기)도 병기로 인정한다', () => {
  const { findings, excluded } = checkItemCite(
    [R('079', '세공 재료가 아닌 재료 아이템(ItemStatRoll 세공 재료 목록에 없는 아이템, 예: 일반 무기 강화 주문서)을 사용하면 세공 화면으로 전환되지 않는지 확인')], DICT);
  assert.strictEqual(has(findings, 'missing_cite').length, 0);
  assert.strictEqual(excluded.already_cited, 1);
});
...
t('제외분을 센다 — 후보 0건이 "위반 없음"인지 "제외가 다 먹었는지" 구분 가능', () => {
  const rows = [
    ...
    R('011', '세공 재료 아이템(예: 세공석)을 사용하면 전환되는지 확인'),
  ];
  const { findings, excluded } = checkItemCite(rows, DICT);
  assert.strictEqual(has(findings, 'missing_cite').length, 0);
