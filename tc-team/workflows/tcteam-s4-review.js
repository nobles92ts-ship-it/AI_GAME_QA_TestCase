...
**⚠ \`${W}/item_cite_report.json\` + \`${W}/item_dict.json\`이 있으면 반드시 Read** — 아이템 실명 병기 검사다(규칙 SSoT=tc-설계.md §아이템 실명 병기).
리포트의 \`detail[]\`:
- \`placeholder_no_index\`·\`unknown_item_name\`·\`cite_with_scope_all\`(HIGH)·\`duplicate_cite\`(MEDIUM)는 **기계가 확정한 위반**이라 드롭 금지 — 전건 edit 패치를 내라.
- \`missing_cite\`는 **위반이 아니라 질문지**다. 그 행을 실행하려면 테스터가 그 아이템을 직접 골라야 하는가(축1)? ①상태로만 등장 ②관측 주체가 장비·버튼·탭 ③유형 전수가 검증 대상 ④한 문장에서 이미 지목 — 이 중 하나면 **그대로 두고**, 아니면 \`유형(예: 실아이템명)\` 병기 edit를 제안하라. 어떤 개체냐가 기대결과를 바꾸면(축2) 기대결과도 그 개체 기준으로 확정하라(예: 무기 강화 주문서 → 무기 탭).
- 예시는 반드시 \`item_dict.json\` 의 \`examples[].cite\`/\`name_index\` 에서 고르라 — 기억·추측으로 아이템명을 쓰면 \`unknown_item_name\`으로 되돌아온다.
- \`zero_combos\`에 있는 조합을 전제한 행은 **delete 하지 말고** J='기획 확인 필요' + 기획 확인 요청 등재를 제안하라 — 그 케이스가 현행 테이블에 없다는 사실이 근거다.
- \`excluded\`가 크고 \`missing_cite\`가 0이면 "위반 없음"이 아니라 제외 규칙이 다 먹었을 수 있다 — 표본 몇 행을 직접 확인하라.` },
