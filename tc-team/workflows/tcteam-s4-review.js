...
- \`missing_cite\`는 **위반이 아니라 질문지**다. 그 행을 실행하려면 테스터가 그 아이템을 직접 골라야 하는가(축1)? ①상태로만 등장 ②관측 주체가 장비·버튼·탭 ③유형 전수가 검증 대상 ④한 문장에서 이미 지목 — 이 중 하나면 **그대로 두고**, 아니면 \`유형(예: 실아이템명)\` 병기 edit를 제안하라. 어떤 개체냐가 기대결과를 바꾸면(축2) 기대결과도 그 개체 기준으로 확정하라(예: 샘플 강화권B → 무기 탭).
