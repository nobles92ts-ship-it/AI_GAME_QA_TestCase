...
Gemma4 (via Ollama) is used as a **fast, free, offline fallback** for tasks where Claude would be overkill:
- Generating first-pass TCs
- Mechanical fixes following a reviewer's prescription
- Local analysis passes

### How it is called

There are two call paths:

1. **MCP server** (`.claude/scripts/mcp-gemma4.js`) — registered as `ask_gemma4` tool. Claude itself can call this like any other tool.
2. **Direct Python helpers** (`tc-team-v2/scripts/gemma4_*.py`) — invoked by the orchestrator via Bash for batch jobs.

Both paths hit the same Ollama HTTP endpoint (`http://localhost:11434/api/generate`).

Model selection is env-driven: `GEMMA4_MODEL` in `.env` is the single source of truth for both Node and Python callers.
